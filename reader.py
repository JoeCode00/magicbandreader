...
try:
    ...
    raise Exception('Ubuntu in python env: apt-get install git, '
                    + 'Windows cmd: winget install --id Git.Git '
                    + '-e --source winget , '
                    + 'Both git bash: git clone '
                    + 'https://github.com/soonuse/pn532-nfc-hat.git')

environ['PYGAME_HIDE_SUPPORT_PROMPT'] = '1'
try:
    import pygame
except Exception:
    raise Exception('pip install pygame')

try:
    import RPi.GPIO as GPIO
except Exception:
    raise Exception('pip install RPi.GPIO')
